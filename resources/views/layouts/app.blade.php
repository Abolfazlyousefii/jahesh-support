...
<body x-data="{ drawer: false }" class="min-h-screen pb-16 lg:pb-0">
    <aside class="fixed inset-y-0 right-0 z-30 hidden w-64 border-l border-gray-200 bg-white p-4 lg:block">
        <div class="mb-6 flex h-12 items-center gap-3 px-2"><span class="grid h-9 w-9 place-items-center rounded-lg bg-brand font-black text-emerald-950">ج</span><div><strong class="block">{{ $generalSettings['general.company_name'] ?? 'تیم جهش' }}</strong><span class="text-xs text-gray-500">{{ $generalSettings['general.app_name'] ?? 'سامانه پشتیبانی جهش' }}</span></div></div>
    ...
    <div x-cloak x-show="drawer" class="fixed inset-0 z-40 lg:hidden" @keydown.escape.window="drawer=false">
        ...
        <aside class="relative h-full w-72 bg-white p-4" x-transition><div class="mb-6 flex h-12 items-center justify-between"><strong>{{ $generalSettings['general.company_name'] ?? 'تیم جهش' }}</strong><button class="h-11 w-11 text-xl" @click="drawer=false" aria-label="بستن">×</button></div>@include('layouts.navigation')</aside>
    </div>
    <div class="lg:mr-64">
        <header class="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-gray-200 bg-white px-4 sm:px-6">
            ...
            <div class="flex items-center gap-3"><x-notification-bell /><div class="hidden text-left sm:block"><strong class="block text-sm">{{ auth()->user()->name }}</strong><span class="text-xs text-gray-500">{{ auth()->user()->roles->pluck('title')->join('، ') ?: 'بدون نقش' }}</span></div><form method="POST" action="{{ route('logout') }}">@csrf<button class="grid h-11 w-11 place-items-center rounded-lg border border-gray-200 text-gray-600" title="خروج"><x-icon name="logout" /></button></form></div>
        </header>
